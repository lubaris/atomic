...
contract AWCstakingV2 is Initializable,  OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable { 
    ...
    function restake() external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(user.isRewardStake, "restake: user amount is too low");
        uint256 pendingReward = (user.amount  * (block.timestamp - user.lastRewardTime) * rewardPerSecond) / 1e18;
    ...
    function unstakeAll() external nonReentrant {
        ...
        require(user.endFreeze <= block.timestamp, "withdraw: freeze time not end");
        if (user.isRewardStake) {
            uint256 pending = (user.amount  * (block.timestamp - user.lastRewardTime) * rewardPerSecond) / 1e18;
            if(pending + user.debt  >= atomicToken.balanceOf(address(pool))) {
                user.debt += pending;
            } else {
                pool.withdrawTokensPool(msg.sender, pending + user.debt);
                user.debt = 0;
            }
        }
